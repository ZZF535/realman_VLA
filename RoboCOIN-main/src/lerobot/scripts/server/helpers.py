...
def raw_observation_to_observation(
    ...
    Args:
        target_state_dim: 如果指定，会自动将 state 补齐到目标维度。
                         用于处理双臂机器人 14维->26维 的情况。
    """
    observation = {}
...
def extract_state_from_raw_observation(
    ...
    如果 target_state_dim 指定且 state 维度不匹配，会自动补齐。
    例如：14维 -> 26维 (双臂机器人，每臂补6个位姿作为占位)
    """
    ...
    # 🚨 维度补齐：14维 -> 26维
    # 适用于双臂机器人（每臂7关节），但模型训练时使用了26维状态
    current_dim = state.shape[-1]
    if target_state_dim is not None and current_dim != target_state_dim:
        if current_dim == 14 and target_state_dim == 26:
            # 双臂结构：[L7, R7] -> [L7, pose6, R7, pose6]
            batch_size = state.shape[0]
            device = state.device
            dtype = state.dtype
            
            left_joints = state[..., :7]   # 左臂 7 关节
            right_joints = state[..., 7:]  # 右臂 7 关节
            
            # 🔥 使用训练数据中的典型位姿值，而不是全0！
            # 这些值来自训练数据的初始帧
            # 左臂位姿: [x, y, z, rx, ry, rz] (米和弧度)
            left_pose = torch.tensor(
                [[0.256, -0.091, -0.441, 3.148, -0.223, -1.357]], 
                device=device, dtype=dtype
            ).expand(batch_size, -1)
            # 右臂位姿: [x, y, z, rx, ry, rz] (米和弧度)
            right_pose = torch.tensor(
                [[-0.245, -0.088, -0.440, -3.127, 0.211, 1.555]], 
                device=device, dtype=dtype
            ).expand(batch_size, -1)
            
            state = torch.cat([left_joints, left_pose, right_joints, right_pose], dim=-1)
            logging.debug(f"State padded from {current_dim} to {state.shape[-1]} dimensions with default poses")
...
def prepare_raw_observation(
    ...
    # 传递 target_state_dim 以支持维度补齐
    state_dict = {OBS_STATE: extract_state_from_raw_observation(lerobot_obs, target_state_dim=target_state_dim)}
    image_dict = {
