...
class RobotClient:
    ...
    def _action_tensor_to_action_dict(self, action_tensor: torch.Tensor) -> dict[str, float]:
        """
        将模型输出的动作张量转换为机器人动作字典。
        
        如果模型输出 26 维 (left_7 + pose_6 + right_7 + pose_6)，
        但机器人只需要 14 维 (left_7 + right_7)，则需要正确提取。
        """
        action_keys = list(self.robot.action_features.keys())
        n_robot_actions = len(action_keys)
        n_model_actions = action_tensor.shape[0]
        
        # 如果模型输出 26 维，机器人需要 14 维，做特殊映射
        if n_model_actions == 26 and n_robot_actions == 14:
            # 26 维格式: left_7 (0-6) + pose_6 (7-12) + right_7 (13-19) + pose_6 (20-25)
            # 提取: left_7 (0-6) + right_7 (13-19)
            left_actions = action_tensor[0:7].tolist()
            right_actions = action_tensor[13:20].tolist()
            extracted_actions = left_actions + right_actions
            action = {key: extracted_actions[i] for i, key in enumerate(action_keys)}
        else:
            # 维度匹配，直接映射
