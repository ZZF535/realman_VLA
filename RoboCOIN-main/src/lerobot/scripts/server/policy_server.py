...
class PolicyServer(services_pb2_grpc.AsyncInferenceServicer):
    ...
    def _prepare_observation(self, observation_t: TimedObservation) -> Observation:
        ...
        """
        # 🚨 获取模型期望的 state 维度，用于自动补齐
        # 例如：双臂机器人返回 14 维，但模型期望 26 维
        target_state_dim = None
        if hasattr(self.policy, 'config') and hasattr(self.policy.config, 'robot_state_feature'):
            state_feature = self.policy.config.robot_state_feature
            if state_feature is not None and hasattr(state_feature, 'shape'):
                target_state_dim = state_feature.shape[0]
                self.logger.debug(f"Target state dimension from policy config: {target_state_dim}")
