...
class BiBaseRobot(Robot):
    ...
    def send_action(self, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # [关键修正] 这里必须通过键名去字典里取值，而不是直接迭代字典
        state_left = {f"left_{k}": state_left[k] for k in self.left_robot._motors_ft.keys()}
        state_right = {f"right_{k}": state_right[k] for k in self.right_robot._motors_ft.keys()}
    ...
    #     import torch
    #     if isinstance(state_left, torch.Tensor) and isinstance(state_right, torch.Tensor):
    #         # 拼成 26 维向量
    #         combined_state = torch.cat([state_left, state_right])
    #         # 包装成模型期望的字典格式
    #         return {"observation.state": combined_state}
    ...
    def get_observation(self) -> dict:
        ...
        # 2. 处理键名 (适配 Realman 返回的字典)
        state_left = {f"left_{k}": v for k, v in state_left.items()}
        state_right = {f"right_{k}": v for k, v in state_right.items()}
        
        # 3. 合并关节数据
        obs = {**state_left, **state_right}
        
        # 4. 📸 核心修复：读取并添加相机图像
        # 检查自身是否有 cameras 属性 (由 RobotConfig 注入)
