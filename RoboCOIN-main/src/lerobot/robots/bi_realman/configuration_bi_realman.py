...
class BiRealmanConfig(BiBaseRobotConfig):
    ...
    ##### Zhixing 90D Gripper Settings (Left Arm) #####
    use_zhixing_gripper_left: bool = True
    gripper_port_left: int = 1
    gripper_baudrate_left: int = 115200
    gripper_device_id_left: int = 1
    gripper_enable_reg_left: int = 256
    gripper_pos_reg_left: int = 259
    gripper_trigger_reg_left: int = 264

    ##### Zhixing 90D Gripper Settings (Right Arm) #####
    use_zhixing_gripper_right: bool = True
    gripper_port_right: int = 1
    gripper_baudrate_right: int = 115200
    gripper_device_id_right: int = 1
    gripper_enable_reg_right: int = 256
    gripper_pos_reg_right: int = 259
    gripper_trigger_reg_right: int = 264
    ...
    #      1.16,  2.01, -0.79, -0.68, -2.84, -1.61,  2.37, 832.00,
    # ])
    init_state_left: List[float] = field(default_factory=lambda: [0.0] * 7)
    init_state_right: List[float] = field(default_factory=lambda: [0.0] * 7)
    # Realman SDK uses degrees for joint angles and meters for positions
    joint_units: List[str] = field(default_factory=lambda: [
    ...
    # 🔥 模型训练时使用的是角度（degree），不是弧度（radian）！
