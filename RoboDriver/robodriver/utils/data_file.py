...
def has_actual_image_data(dir_path):
    """
    递归检查目录中是否存在实际的图片文件（支持多级目录）
    :param dir_path: 要检查的目录路径
    :return: 存在有效图片返回 True，否则返回 False
    """
    if not os.path.isdir(dir_path):
        return False  # 不是目录，直接返回False
        ...
        for entry in entries_1:
            ...
            if entry == "videos":
                if "images" in entries_1:
                    image_dir = os.path.join(task_path, "image")
                    if has_actual_image_data(image_dir):
                        continue
                data_path = os.path.join(task_path, entry, "chunk-000")
                size_bytes += file_size(data_path, episode_index)
            if entry == "images":
                image_dir = os.path.join(task_path, "image")
                if not has_actual_image_data(image_dir):
                        continue
