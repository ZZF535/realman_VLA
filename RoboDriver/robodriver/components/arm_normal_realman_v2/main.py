...
class RealmanArm:
    ...
    #     tag, gripper_dict = self.arm.rm_get_rm_plus_state_info()
    #     gripper_actpos = np.array([gripper_dict['pos'][0]]).astype(np.float64)
    #     return gripper_actpos
    
    def rm_read_gripper_actpos(self):
        tag, state_info = self.arm.rm_get_rm_plus_state_info()
        gripper_actpos = np.array([state_info['pos'][0]]).astype(np.float64)
        return gripper_actpos
